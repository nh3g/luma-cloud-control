-- Corrige advertências: funções auxiliares como SECURITY INVOKER
...
BEGIN
  IF EXISTS (SELECT 1 FROM public.campaigns WHERE workspace_id = _ws) THEN RETURN; END IF;
  ...
  FOR c IN SELECT * FROM public.campaigns WHERE workspace_id = _ws LOOP
    FOR d IN 0..13 LOOP
  ...
  -- Decisões cobrindo cada estado e cada tipo de ação
  ...
  UPDATE public.decisions SET status = 'EXPIRED', expires_at = now() - interval '3 hours'
  WHERE id = dec4 AND false;

  -- Decisão expirada (validade no passado, aplicada após a inserção)
  INSERT INTO public.decisions (workspace_id, platform, account_id, campaign_id, campaign_name, action_type, reason,
    previous_value_json, proposed_value_json, confidence, risk_level, status, source, expires_at)
...
BEGIN
  ...
  IF ws IS NULL THEN
    SELECT id INTO ws FROM public.workspaces WHERE owner_id = NEW.id;
  END IF;

  INSERT INTO public.engine_settings (workspace_id) VALUES (ws) ON CONFLICT (workspace_id) DO NOTHING;
  PERFORM public.seed_demo_workspace(ws);
  RETURN NEW;
END;
$$;

-- Aplica a workspaces já existentes
DO $$
DECLARE w record;
BEGIN
  FOR w IN SELECT id FROM public.workspaces LOOP
    INSERT INTO public.engine_settings (workspace_id) VALUES (w.id) ON CONFLICT (workspace_id) DO NOTHING;
